#include "secure-network-scheduler.h"

namespace ns3 {
namespace lorawan {

NS_LOG_COMPONENT_DEFINE ("SecureNetworkScheduler");

NS_OBJECT_ENSURE_REGISTERED (SecureNetworkScheduler);

TypeId
SecureNetworkScheduler::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SecureNetworkScheduler")
    .SetParent<Object> ()
    .AddConstructor<SecureNetworkScheduler> ()
...
SecureNetworkScheduler::SecureNetworkScheduler ()
{
}

SecureNetworkScheduler::SecureNetworkScheduler (Ptr<SecureNetworkStatus> status,
...
void
SecureNetworkScheduler::OnReceivedPacket (Ptr<const Packet> packet)
{
...
void
SecureNetworkScheduler::OnReceiveWindowOpportunity (LoraDeviceAddress deviceAddress, int window)
{
  ...
  if (gwAddress == Address () && window == 1)
    {
      ...
      // Schedule OnReceiveWindowOpportunity event
      Simulator::Schedule (Seconds (1),
                           &SecureNetworkScheduler::OnReceiveWindowOpportunity,
                           this,
                           deviceAddress,
