...
#ifndef SECURE_NETWORK_STATUS_H
#define SECURE_NETWORK_STATUS_H
 ...
 */
class SecureNetworkStatus : public Object
{
public:
  static TypeId GetTypeId (void);

  SecureNetworkStatus ();
  virtual ~SecureNetworkStatus ();

  /**
   * Add a device to the ones that are tracked by this SecureNetworkStatus object.
   */
  void AddNode (Ptr<SecureEndDeviceLorawanMac> edMac);
   ...
   */
  Ptr<SecureEndDeviceStatus> GetEndDeviceStatus (Ptr<Packet const> packet);

  /**
   * Get the EndDeviceStatus corresponding to a LoraDeviceAddress.
   */
  Ptr<SecureEndDeviceStatus> GetEndDeviceStatus (LoraDeviceAddress address);
...
public:
  std::map<LoraDeviceAddress, Ptr<SecureEndDeviceStatus>> m_endDeviceStatuses;
