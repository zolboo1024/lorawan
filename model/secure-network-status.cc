...
NS_LOG_COMPONENT_DEFINE ("SecureNetworkStatus");

NS_OBJECT_ENSURE_REGISTERED (SecureNetworkStatus);

TypeId
SecureNetworkStatus::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SecureSecureNetworkStatus")
    .AddConstructor<SecureNetworkStatus> ()
    .SetGroupName ("lorawan");
  return tid;
}

SecureNetworkStatus::SecureNetworkStatus ()
{
...
void
SecureNetworkStatus::AddNode (Ptr<SecureEndDeviceLorawanMac> edMac)
{
  ...
  if (m_endDeviceStatuses.find (edAddress) == m_endDeviceStatuses.end ())
    {
      ...
      Ptr<SecureEndDeviceStatus> edStatus = CreateObject<SecureEndDeviceStatus>
        (edAddress, edMac->GetObject<SecureEndDeviceLorawanMac>());

      // Add it to the map
      m_endDeviceStatuses.insert (std::pair<LoraDeviceAddress, Ptr<SecureEndDeviceStatus> >
                                  (edAddress, edStatus));
...
void
SecureNetworkStatus::AddGateway (Address& address, Ptr<GatewayStatus> gwStatus)
{
...
void
SecureNetworkStatus::OnReceivedPacket (Ptr<const Packet> packet,
                                  const Address& gwAddress)
{
  ...
  // Update the correct SecureEndDeviceStatus object
...
bool
SecureNetworkStatus::NeedsReply (LoraDeviceAddress deviceAddress)
{
...
Address
SecureNetworkStatus::GetBestGatewayForDevice (LoraDeviceAddress deviceAddress, int window)
{
  // Get the endDeviceStatus we are interested in
  Ptr<SecureEndDeviceStatus> edStatus = m_endDeviceStatuses.at (deviceAddress);
...
void
SecureNetworkStatus::SendThroughGateway (Ptr<Packet> packet, Address gwAddress)
{
...
Ptr<Packet>
SecureNetworkStatus::GetReplyForDevice (LoraDeviceAddress edAddress, int windowNumber)
{
  // Get the reply packet
  Ptr<SecureEndDeviceStatus> edStatus = m_endDeviceStatuses.find (edAddress)->second;
...
Ptr<SecureEndDeviceStatus>
SecureNetworkStatus::GetEndDeviceStatus (Ptr<Packet const> packet)
{
...
Ptr<SecureEndDeviceStatus>
SecureNetworkStatus::GetEndDeviceStatus (LoraDeviceAddress address)
{
...
int
SecureNetworkStatus::CountEndDevices (void)
{
  NS_LOG_FUNCTION (this);
