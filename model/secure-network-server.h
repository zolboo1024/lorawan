...
#ifndef SECURE_NETWORK_SERVER_H
#define SECURE_NETWORK_SERVER_H
...
/**
 * The SecureNetworkServer is an application standing on top of a node equipped with
 * links that connect it with the gateways.
 *
 * This version of the SecureNetworkServer attempts to closely mimic an actual
 * Network Server, by providing as much functionality as possible.
 */
class SecureNetworkServer : public Application
{
public:
  static TypeId GetTypeId (void);

  SecureNetworkServer ();
  virtual ~SecureNetworkServer ();
  ...
  /**
   * Inform the SecureNetworkServer that these nodes are connected to the network.
   *
  ...
  /**
   * Inform the SecureNetworkServer that this node is connected to the network.
  ...
  /**
   * A NetworkControllerComponent to this SecureNetworkServer instance.
   */
  ...
  Ptr<SecureNetworkStatus> GetNetworkStatus (void);
